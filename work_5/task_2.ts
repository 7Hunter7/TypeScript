...
5) Примечание: 
- OldUser и NewUser полностью идентичны, их разделение здесь бессмысленно.
- В задании требуется использовать дженерики, но методы getUsers(), getNewUsers() и getSomeOther() жестко привязаны к конкретным типам (OldUser, NewUser).
- Метод getSomeOther() не использует дженерики, просто берет data из getNewUsers(), но без явного указания типа через дженерик
- Тут пример решения, смотреть в случае если будут сложности https://codepen.io/Quper/pen/jEOBOQO
 */
